...
def run(params, data_dir, output_path):
    ...
    fns = sorted([fn for fn in os.listdir(data_dir) if fn.endswith('.tif') and 'REGISTERED_DOWNSAMPLED-8x' in fn])
    ...
    if not fns:
        ...
        for fn in fns:
            if 'REGISTERED' not in fn:
                ...
                params['IMsz'] = extract_pixel_resolution(metadata_str, [125, 45])[::-1]  # row, column instead XY
            ...
            if 'REGISTERED_DOWNSAMPLED-8x' in fn:
                ...
                A = tifffile.imread(os.path.join(data_dir, fn))
                A = A.transpose(1, 2, 0)
                ...
                for trialIx in tqdm(range(1, params['numTrials']+1), desc='Simulation Progress'):
                    ...
                    Ad = np.zeros((len(selR), len(selC), numChannels, params['T']), dtype=np.float32)
                    ...
                    for frameIx in range(params['T']):
                        ...
                        excessNoise = np.clip(1 + params['excessNoiseSigma'] *
                                              np.random.randn(selR.size, selC.size), 0.5, 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--brightness', type=float, default=5,
    ...
    parser.add_argument('--photonScale', type=int, default=300,
                        help='Amplitude of a single photon in digitizer units.')  # Wont vary in practice
    parser.add_argument('--excessNoiseSigma', type=float, default=.1,
                        help='Std.dev. of the excess multiplicative noise factor related to detector gain variation')
